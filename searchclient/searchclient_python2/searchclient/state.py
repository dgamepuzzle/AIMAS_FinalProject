    ...
    def __init__(self, copy: 'State' = None):
        ...
        if copy is None:
            ...
            if State.walls is None:
                State.walls = [[False for _ in range(State.maxCol)] for _ in range(State.maxRow)]
            
            if State.goals is None:
                State.goals = [[None for _ in range(State.maxCol)] for _ in range(State.maxRow)]
            
            #self.walls = [[False for _ in range(State.MAX_COL)] for _ in range(State.MAX_ROW)]
            self.boxes = [[None for _ in range(State.maxCol)] for _ in range(State.maxRow)]
